...
class NBNoVNC(Configurable):
    ...
    vnc_command = Unicode(u"xinit -- /usr/bin/Xtigervnc :{display} -geometry {geometry} -depth {depth}  -SecurityTypes None",
                          config=True, help="Command to start VNC server. Contains string replacement fields.")
    websockify_command = Unicode(u"websockify --web {novnc_directory} --heartbeat {heartbeat} {port} localhost:{vnc_port}",
                                 config=True,
